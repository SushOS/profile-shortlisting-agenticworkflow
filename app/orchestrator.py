...
# class Orchestration:
#     """
#     Conditional orchestration pattern with intelligent routing based on state.
#     """
...
#         self.graph.add_node("score", self._score_node)
#         self.graph.add_node("sheet", self._sheet_node)
#         self.graph.add_node("retry", self._retry_node)
#         self.graph.add_node("error", self._error_node)
...
#             {
#                 "score": "score",
#                 "search": "search",  # Retry search if no data
#                 "error": "error"
#             }
#         )
...
#             {
#                 "sheet": "sheet",
#                 "search": "search",  # Retry if no good candidates
#                 "error": "error"
#             }
#         )
...
#         # Direct edges for final states
#         self.graph.add_edge("sheet", "__end__")
#         self.graph.add_edge("error", "__end__")
...
#             return state
#         except Exception as e:
#             state["error_message"] = f"Sheet writing failed: {str(e)}"
#             return state

#     def _retry_node(self, state: State) -> State:
#         """Handle retry logic"""
#         state["retry_count"] = state.get("retry_count", 0) + 1
#         print(f"Retrying... Attempt {state['retry_count']}")
#         return state

#     def _error_node(self, state: State) -> State:
#         """Handle final error state"""
#         state["result"] = f"Workflow failed: {state['error_message']}"
#         print(f"Error: {state['error_message']}")
#         return state

#     # Routing Functions
#     def _route_after_analyze(self, state: State) -> Literal["search", "error", "retry"]:
#         """Route after analysis based on spec quality"""
...
#         if not spec.get("keywords") and not spec.get("education"):
#             if state.get("retry_count", 0) < 2:
#                 return "retry"
#             return "error"
...
#     def _route_after_search(self, state: State) -> Literal["enrich", "retry", "error"]:
#         """Route after search based on results quality"""
#         if state.get("error_message"):
#             if state.get("retry_count", 0) < 2:
#                 return "retry"
#             return "error"
...
#         # If no results, retry with different search strategy
#         if len(snippets) == 0:
#             if state.get("retry_count", 0) < 2:
#                 return "retry"
#             return "error"
...
#     def _route_after_enrich(self, state: State) -> Literal["score", "search", "error"]:
#         """Route after enrichment based on data quality"""
#         if state.get("error_message"):
#             return "error"
        
#         df = state.get("df")
        
#         # If dataframe is empty or None, try search again
#         if df is None or df.empty:
#             if state.get("retry_count", 0) < 2:
#                 return "search"
#             return "error"
        
#         # If very few results, try search again with broader criteria
#         if len(df) < 3:
#             if state.get("retry_count", 0) < 1:  # Only retry once for low results
#                 return "search"
        
#         return "score"

#     def _route_after_score(self, state: State) -> Literal["sheet", "search", "error"]:
#         """Route after scoring based on candidate quality"""
#         if state.get("error_message"):
#             return "error"
...
#         # Check if we have any high-quality candidates (score > 5)
#         high_quality_candidates = scores[scores['score'] > 5] if 'score' in scores.columns else scores
        
#         if len(high_quality_candidates) == 0:
#             if state.get("retry_count", 0) < 1:
#                 print("No high-quality candidates found. Retrying with broader search...")
#                 return "search"
        
#         return "sheet"

#     def _route_after_retry(self, state: State) -> Literal["analyze", "search", "error"]:
#         """Route after retry based on what failed"""
...
#             return "analyze"
#         elif "Search failed" in error_msg or "No results" in error_msg:
#             return "search"
